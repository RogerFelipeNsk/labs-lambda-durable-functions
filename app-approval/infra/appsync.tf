...
resource "aws_appsync_datasource" "solicitacoes" {
  api_id           = aws_appsync_graphql_api.principal.id
  name             = "tabela_solicitacoes"
  type             = "AMAZON_DYNAMODB"
  service_role_arn = aws_iam_role.appsync_dynamo.arn

  dynamodb_config {
    table_name = aws_dynamodb_table.solicitacoes.name
    region     = local.regiao
  }
}
...
locals {
  resolvers_dynamo = {
    listSolicitacoes = { tipo = "Query", arquivo = "listSolicitacoes.js" }
    getSolicitacao   = { tipo = "Query", arquivo = "getSolicitacao.js" }
    listEvents       = { tipo = "Query", arquivo = "listEvents.js" }
  }
  resolvers_locais = ["publishSolicitacao", "publishEvent"]
}
...
resource "aws_appsync_resolver" "dynamo" {
  ...
  field       = each.key
  data_source = aws_appsync_datasource.solicitacoes.name
