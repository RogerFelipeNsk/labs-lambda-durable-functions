...
output "tabela_boletos" {
  description = "Nome da tabela do DynamoDB."
  value       = aws_dynamodb_table.boletos.name
}
...
output "proximos_passos" {
  ...
  value       = <<-TXT
    ...
    2. abra http://localhost:5173  (o token da API ja foi para o .env.local)
    3. emita um boleto, clique em "Registrar pagamento" e acompanhe a timeline chegando por subscription
    4. compare com a aba "Durable executions" no console: ${local.console_durable}
  TXT
}
...
output "nome_funcao_orquestrador" {
  description = "Nome da durable function, util para 'aws logs tail'."
