...
data "aws_iam_policy_document" "appsync_dynamo" {
  statement {
    ...
    resources = [aws_dynamodb_table.boletos.arn, "${aws_dynamodb_table.boletos.arn}/index/*"]
  }
}

resource "aws_iam_role_policy" "appsync_dynamo" {
  name   = "leitura-boletos"
...
data "aws_iam_policy_document" "tabela_boletos" {
  statement {
    sid = "EscritaELeituraDosBoletos"
    actions = [
      "dynamodb:GetItem",
      "dynamodb:PutItem",
      "dynamodb:UpdateItem",
...
data "aws_iam_policy_document" "publicar_appsync" {
  statement {
    ...
    resources = [
      "${aws_appsync_graphql_api.principal.arn}/types/Mutation/fields/publishBoleto",
...
# das permissoes basicas de CloudWatch Logs. Sem ela a funcao ate inicia, mas
# morre no primeiro step.
...
data "aws_iam_policy_document" "orquestrador" {
  source_policy_documents = [
    data.aws_iam_policy_document.tabela_boletos.json,
...
data "aws_iam_policy_document" "api_durable" {
  ...
  # ARN da versao da funcao. Um ARN sem qualificador nunca casaria, por isso o
  # ":*" — que aqui cobre qualquer versao publicada.
  statement {
