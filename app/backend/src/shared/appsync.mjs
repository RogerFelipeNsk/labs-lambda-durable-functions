 ...
 * entra so como canal de tempo real — as mutations `publishBoleto` e
...
const MUTATION_BOLETO = /* GraphQL */ `
  mutation PublishBoleto($input: BoletoInput!) {
    publishBoleto(input: $input) {
      id
      sacado
      descricao
      valorCentavos
...
/** Campos do item do DynamoDB que existem no schema GraphQL de Boleto. */
const CAMPOS_BOLETO = [
  "id", "sacado", "descricao", "valorCentavos", "status", "stage", "nossoNumero",
  "linhaDigitavel", "createdAt", "updatedAt", "executionArn", "callbackId", "attempt",
  "chaos", "chaosRestante", "pagoEm", "baixadoEm", "comprovanteBaixa", "extratoId",
  "conciliacaoId", "valorLiquidoCentavos", "tarifaCentavos", "divergenciaCentavos", "erro",
];

export async function publicarBoleto(item) {
  const input = limpar(Object.fromEntries(CAMPOS_BOLETO.map((c) => [c, item[c]])));
  await executar(MUTATION_BOLETO, { input });
}
